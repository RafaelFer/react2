...
export default class Timeline extends Component {
    ...
    carregaFotos(){  
      ...
      if(this.login === undefined) {
        urlPerfil = `//instalura-api.herokuapp.com/api/fotos?X-AUTH-TOKEN=${localStorage.getItem('auth-token')}`;
      } else {
        urlPerfil = `//instalura-api.herokuapp.com/api/public/fotos/${this.login}`;
      }
    ...
    like(fotoId) {
      fetch(`//instalura-api.herokuapp.com/api/fotos/${fotoId}/like?X-AUTH-TOKEN=${localStorage.getItem('auth-token')}`,{method:'POST'})
        .then(response => {
          if(response.ok) {
    ...
    comenta(fotoId,textoComentario) {
      ...
      fetch(`//instalura-api.herokuapp.com/api/fotos/${fotoId}/comment?X-AUTH-TOKEN=${localStorage.getItem('auth-token')}`,requestInfo)
        .then(response => {
          if(response.ok){
            return response.json();
          } else {
